...
{
    public abstract class BitmapQuantizerBase
    {
        ...
        public BitmapOutput QuantizeImage(BitmapInput image) { return QuantizeImage(image, 10, 70, 256); }
        public BitmapOutput QuantizeImage(BitmapInput image, int alphaThreshold, int alphaFader) { return QuantizeImage(image, alphaThreshold, alphaFader, 256); }

        public BitmapOutput QuantizeImage(BitmapInput image, int alphaThreshold = 10, int alphaFader = 70, int maxColors = 256)
        {
            ...
            return GetQuantizedImage(image, colorCount, maxColors, lookups, alphaThreshold);
        }

        private static ColorMoment[,,,] BuildHistogram(BitmapInput sourceImage, int alphaThreshold, int alphaFader)
        {
        ...
        internal abstract BitmapOutput GetQuantizedImage(BitmapInput image, int colorCount, int maxColors, Pixel[] lookups, int alphaThreshold);
    }
}
